...
public class ScoreMain {
	...
	public static void main(String[] args) {
		...
		for(int i =0; i<100;i++) {
			...
			System.out.print((i+1)+ "번째 학생 [이름]을 입력해주세요 : ");
			String n = sc.next();
			if(n.equals("그만"))break;		
			System.out.print((i+1)+ "번째 학생 [국어점수]를 입력해주세요 : ");
			int k = sc.nextInt();
			System.out.print((i+1)+ "번째 학생 [수학점수]를 입력해주세요 : ");
			int m = sc.nextInt();
			System.out.print((i+1)+ "번째 학생 [영어점수]를 입력해주세요 : ");
			int e = sc.nextInt();
			Score s = new Score(n,k,m,e);
			stu[i]=s;
		}
		for(int j=0; j<100;j++) {
			if(stu[j]==null)break;
